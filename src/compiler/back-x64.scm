;; back-x64.scm -- native compiler backend x64
...
(define (compile-if exp env si dst)
  (let ((alt-label (unique-label))
    ...
    (emit
     (compile (if-test exp) env si %rax)
     (cmpq ($ (encode #f)) %al)
     (je alt-label)
...
(define (compile-global-variable binding si dst)
  (let ((end-label (unique-label)))
    ...
    (emit
     ...
     (movq (^ (- (* 2 $wordsize) $ptr-tag) dst) dst)
     (cmpq ($ $immediate-unbound) dst)
     (je end-label)
     end-label)))
...
(define (compile-set! exp env si dst)
  (let ((binding (lookup-variable (set!-variable exp) env)))
	...
	(emit
	 (compile (set!-value exp) env si %rax)
	 (emit-free-load (global-binding-depth binding) %rbx)
	 (mov1 %rax (^ (- (* 2 $wordsize) $ptr-tag) %rbx)))
...
(define (compile-lambda-free-var var env si offset)
  (emit (compile-variable (cadr var) env si %rax)
	(movq %rax (^ offset %rbp))))
...
(define (compile-lambda-free-global var env si offset)
  (let ((binding (lookup-variable (cadr var) env)))
    (if (and binding (global-binding? binding))
	(emit 
	 (emit-free-load (global-binding-depth binding) %rax)
	 (movq %rax (^ offset %rbp)))
...
(define (compile-lambda exp env si dst)
  (let* ((free (lookup-lambda-free exp (env-free env)))
    ...
    (emit
     (movq ($ (header (+ (length free) 2) 0)) (^ 0 %rbp))
     (emit-free-load depth %rax)
     (addq ($ 7) %rax)
     (movl %rax (^ $wordsize %rbp))
     (compile-lambda-free free env si (* 2 $wordsize))
     (movq %rbp dst)
     (orq ($ $closure-tag) dst)
     (addq ($ (* $wordsize (+ 2 free-len))) %rbp))))
...
(define (compile-call exp env si dst)
  (let* ((arguments (call-arguments exp))
    ...
    (emit
     (movq %rsi (^ saved-closure-pointer-si %rsp))
     (compile-arguments (call-arguments exp) env args-si)
     (compile (call-target exp) env si %rsi)
     (movq %rsi %rax)
     (andq ($ $primary-mask) %rax)
     (cmp ($ $closure-tag) %al)
     (je call-label)
     call-label
     (movq ($ (encode argument-count)) %rdx)
     (emit-adjust-base si)
     (movq (^ (- $wordsize $closure-tag) %rsi) %rax)
     (addq ($ (- 0 $closure-tag)) %rsi)
     (call-*rax)
     (emit-adjust-base (- 0 si))
     (movq (^ saved-closure-pointer-si %rsp) %rsi))))
