...
(define (x86-movl-old src dst)
  (cond ((and (x86-immediate? src) (eq? dst %eax))
	...
	((and (eq? src %eax)
	      (x86-indirect? dst)
	      (eq? %ebx (x86-indirect-register dst)))
	 (list #x89 #x43 (x86-encode-8 (x86-indirect-offset dst))))
	...
	((and (x86-indirect? src)
	      (eq? %esi (x86-indirect-register src))
	      (eq? %ebx dst))
	 (list #x8b #x5e (x86-encode-8 (x86-indirect-offset src))))
	((and (x86-indirect? src)
	...
	((and (x86-indirect? src)
	      (eq? (x86-indirect-register src) %esp)
	      (eq? %edx dst))
	 (list #x8b #x54 #x24 (x86-encode-8 (x86-indirect-offset src))))
	((and (x86-indexed? src)
	      (eq? (x86-indexed-base src) %eax)
	      (eq? (x86-indexed-offset src) %edx)
	      (eq? %eax dst))	 
	 (list #x8b #x44 #x90 (x86-encode-8 (x86-indexed-scale src))))
	((and (x86-indexed? dst)
	      (eq? (x86-indexed-base dst) %eax)
	      (eq? (x86-indexed-offset dst) %edx)
	      (eq? %ecx src))	 
	 (list #x89 #x0c #x90 (x86-encode-8 (x86-indexed-scale dst))))
        (else (error "movl not supported: movl ~s ~s" src dst))))

(define (x86-modr/m reg mod r/m)
  (+ (* mod 64)
     (+ (* reg 8) r/m)))

(define (x86-sib base index scale)
  (+ (* scale 64)
     (+ (* index 8)
	base)))

(define (x86-movl src dst)
  (cond ((and (x86-immediate? src) (x86-register? dst))
	 (list (+ #xb8 dst) (x86-encode-32 (x86-immediate-value src))))
	((and (x86-immediate? src) (x86-indirect? dst))
	 (list #xc7 (x86-modr/m 0 1 (x86-indirect-register dst))
	       (x86-encode-8 (x86-indirect-offset dst))
	       (x86-encode-32 (x86-immediate-value src))))
	((and (x86-register? src) (x86-register? dst))
	 (list #x89 (x86-modr/m src 3 dst)))
	((and (x86-register? src) (x86-indirect? dst))
	 (if (eq? %esp (x86-indirect-register dst))
	     (list #x89 (x86-modr/m src 1 (x86-indirect-register dst))
		   #x24
		   (x86-encode-8 (x86-indirect-offset dst)))
	     (list #x89 (x86-modr/m src 1 (x86-indirect-register dst))
		   (x86-encode-8 (x86-indirect-offset dst)))))
	((and (x86-indirect? src) (x86-register? dst))
	 (if (eq? %esp (x86-indirect-register src))
	     (list #x8b (x86-modr/m dst 1 (x86-indirect-register src))
		   #x24
		   (x86-encode-8 (x86-indirect-offset src)))
	     (list #x8b (x86-modr/m dst 1 (x86-indirect-register src))
		   (x86-encode-8 (x86-indirect-offset src)))))
	((and (x86-register? src) (x86-indexed? dst))
	 (list #x89 (x86-modr/m src 0 4) (x86-sib (x86-indexed-base dst)
						  (x86-indexed-offset dst)
						  2)
	       (x86-encode-8 (x86-indexed-scale dst))))
	((and (x86-indexed? src) (x86-register? dst))
	 (list #x8b (x86-modr/m dst 0 4) (x86-sib (x86-indexed-base src)
						  (x86-indexed-offset src)
						  2)
	       (x86-encode-8 (x86-indexed-scale src))))
	(else (error "movl from ~a to ~a not supported" src dst))))
...
(define (x86-cmpl src dst)
  (cond ((and (x86-immediate? src) (eq? dst %eax))
	...
	((and (x86-immediate? src) (eq? dst %esi))
	 (list #x83 #xfe (x86-encode-8 (x86-immediate-value src))))
...
(define (x86-sar src dst)
  (cond ((and (x86-immediate? src) (eq? dst %edx))
	 (list #xc1 #xfa (x86-encode-8 (x86-immediate-value src))))
	(else (error "unsupported syntac for sar ~a ~a" src dst))))
...
(define (x86-indexed? ind) (and (pair? ind) (eq? (car ind) '^^)))
(define (x86-indexed-base ind) (cadr ind))
(define (x86-indexed-offset ind) (caddr ind))
(define (x86-indexed-scale ind) (cadddr ind))

(define ($ val) (list '$ val))
(define (^ offset reg) (list '^ offset reg))
(define (^^ base offset scale) (list '^^ base offset scale))
