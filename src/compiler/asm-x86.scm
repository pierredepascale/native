...
(define (x86-movl src dst)
  (cond ((and (x86-immediate? src) (eq? dst %eax))
	...
	((and (x86-indirect? src)
	      ...
	      (eq? %ecx dst))
	 (list #x8b #x4c #x24 (x86-encode-8 (x86-indirect-offset src))))
	((and (x86-indirect? src)
	      (eq? (x86-indirect-register src) %esp)
	      (eq? %esi dst))
	 (list #x8b #x74 #x24 (x86-encode-8 (x86-indirect-offset src))))
	((and (x86-indirect? src)
	      (eq? (x86-indirect-register src) %esp)
	      (eq? %ebp dst))
	 (list #x8b #x6c #x24 (x86-encode-8 (x86-indirect-offset src))))
	((and (eq? src %ecx)
	      (x86-indirect? dst)
	      (eq? %esp (x86-indirect-register dst)))
	 (list #x89 #x4c #x24 (x86-encode-8 (x86-indirect-offset dst))))
	((and (eq? src %esi)
	      (x86-indirect? dst)
	      (eq? %esp (x86-indirect-register dst)))
	 (list #x89 #x74 #x24 (x86-encode-8 (x86-indirect-offset dst))))
	((and (eq? src %ebp)
	      (x86-indirect? dst)
	      (eq? %esp (x86-indirect-register dst)))
	 (list #x89 #x6c #x24 (x86-encode-8 (x86-indirect-offset dst))))
